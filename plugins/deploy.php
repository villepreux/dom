...
function arg_state($flag, $on = 1, $off = 0) { global $argv; return (DEPLOY_CLI ? in_array("--$flag",  $argv) : (array_key_exists($flag, $_GET) && !!$_GET[$flag] )) ? $on : $off; }
    ...
    while (count($roots) > 0)
    {
        ...
        foreach (deploy_scan($src) as $name)
        {
            ...
            ||  $extension == "js")
            {
                ...
                {
                    $t_from = filemtime("$src/$name");
                    $t_to   = is_file("$dst/$deploy_name") ? ("$dst/$deploy_name") : 0;
